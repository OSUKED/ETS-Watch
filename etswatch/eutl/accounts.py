...
def get_installation_dfs(df_search=None, data_dir='data/installations', redownload=False):
    if not os.path.exists(data_dir):
        os.makedirs(data_dir)

    if redownload == True:
        if df_search is None:
            df_search = get_search_df(data_dir=f'{data_dir}/..', redownload=True)
        ...
        for filename in filenames:
            installation_dfs[filename] = pd.read_csv(f'{data_dir}/{filename}.csv').set_index('account_id')
...
def get_aircraft_dfs(df_search=None, data_dir='data/aircraft', redownload=False):
    if not os.path.exists(data_dir):
        os.makedirs(data_dir)
    ...
    if redownload == True:
        ...
        for filename in filenames:
            aircraft_dfs[filename] = pd.read_csv(f'{data_dir}/{filename}.csv').set_index('account_id')
